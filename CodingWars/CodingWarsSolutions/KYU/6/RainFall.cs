...
namespace CodingWarsSolutions
{
    class Rainfall
    {
        static Regex NumbersOnly = new Regex(@"[^0-9\.]", RegexOptions.Compiled);

        static double[] getTownRainFallData(string town, string data)
        {
            var townData = data.Split('\n').FirstOrDefault(x => x.Split(':')[0] == town);
            if (townData != null)
                return townData.Split(':')[1].Split(',').Select(x => double.Parse(NumbersOnly.Replace(x, ""))).ToArray();
            return new[] { -1d };
        }

        public static double Mean(string town, string data) =>
            getTownRainFallData(town, data).Average();

        public static double Variance(string town, string data)
        {
            double[] rainfall = getTownRainFallData(town, data);
            if (rainfall.Length == 1 && rainfall[0] == -1)
                return -1;

            double average = rainfall.Average();
            return rainfall.Select(x => Math.Pow(average - x, 2)).Sum() / rainfall.Count();
        }
    }
    ...
    public static class RainfallTests
    {
        ...
        [Test]
        public static void RainTestMean()
        {
        ...
        [Test]
        public static void RainTestVariance()
        {
